...
ebr_volume_id:              db 24h, 32h, 52h, 78h   ; serial number, values doesn't matter
ebr_volume_label:           db 'MY OWN OS  '        ; 11 bytees, padded with spaces 
...
.after:
  ...
  ; compute LBA of root directory = reserved + fats * sectors_per fat
...
.load_kernel_loop:
  ...
  MUL cx
  mov cx, 2
  div cx                            ; ax = index of entriy in FAT, dx = cluster mod 2

  mov si, buffer
  add si, ax
  mov ax, [ds:si]                   ; read entry from FAT table index ax
...
.read_finish:
  ...
  jmp wait_key_and_reboot           ; shouldn't never happen
